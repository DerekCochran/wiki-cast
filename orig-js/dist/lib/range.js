 ...
 */
let AstRange = (() => {
    let _classDecorators = [nodeLike_1.nodeLike, elementLike_1.elementLike];
    ...
    var AstRange = class {
        ...
        static {
            ...
            AstRange = _classThis = _classDescriptor.value;
        ...
        insertNode(newNode, after) {
            if (after) {
                ...
                if (endContainer.type === 'text') {
                    ...
                    endContainer.insertAt(newNode, endOffset);
                }
            }
            else {
                const { startContainer, startOffset } = this;
        ...
        replaceWith(...nodes) {
            this.deleteContents();
            this.#after(nodes);
            const { endContainer } = this, { type, parentNode } = endContainer;
            if (type === 'text') {
                this.#startContainer = parentNode;
        ...
        extractContents() {
            ...
            this.#splitText();
            this.#splitText(true);
            const { startContainer, startOffset, endContainer, endOffset, commonAncestorContainer } = this, { type, childNodes } = commonAncestorContainer;
            if (type === 'text') {
                return [commonAncestorContainer];
            }
            const from = startContainer.type === 'text' ? childNodes.indexOf(startContainer) : startOffset, to = endContainer.type === 'text' ? childNodes.indexOf(endContainer) + 1 : endOffset;
