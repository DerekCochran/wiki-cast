...
function urlFunction(config, args, local) {
    ...
    const title = index_1.default.normalizeTitle(value, 0, false, config, { halfParsed: true, decode: true, temporary: true, page: '' });
    if (!title.valid) {
        return fallback;
    }
    else if (title.ns === -2) {
...
const parseUrl = ({ testServer = '', articlePath = testServer }) => {
...
}, currentMonth1 = (now) => String(now.getUTCMonth() + 1), currentDay = (now) => String(now.getUTCDate()), currentHour = (now) => String(now.getUTCHours()).padStart(2, '0'), currentWeek = (now, firstDay) => String(Math.ceil((now.getTime() - firstDay.getTime()) / 1e3 / 60 / 60 / 24 / 7)), localYear = (now) => String(now.getFullYear()), localMonth1 = (now) => String(now.getMonth() + 1), localMonth = (now) => localMonth1(now).padStart(2, '0'), localDay = (now) => String(now.getDate()), localDay2 = (now) => localDay(now).padStart(2, '0'), localHour = (now) => String(now.getHours()).padStart(2, '0'), localMinute = (now) => String(now.getMinutes()).padStart(2, '0'), ns = ({ nsid, namespaces }, args) => {
    const [val] = args;
    if (val === undefined) {
        return '';
    }
...
}, makeTitle = (page, config, nsid = 0, subpage) => {
    if (page.includes('\0')) {
        return '';
    }
    const title = index_1.default.normalizeTitle(page, nsid, false, config, { halfParsed: true, temporary: true, page: subpage ? undefined : '' });
    return title.valid ? title : '';
}, namespace = (title, config) => config.namespaces[title.ns], namespacee = (title, config) => wfUrlencode(namespace(title, config)), dictHtml1 = {
    '"': '&#34;',
    '&': '&#38;',
    "'": '&#39;',
    '=': '&#61;',
    ...
    'PMID ': 'PMID&#32;',
    'RFC ': 'RFC&#32;',
}, dictHtml2 = { '+': '&#43;', '-': '&#45;', _: '&#95;', '~': '&#126;' }, dictHtml3 = { _: '&#95;', '~': '&#126;' }, wfEscapeWikiText = (text, config) => {
    if (!text) {
        return '';
    }
    ...
    output = output.slice(0, -1)
        + output.at(-1).replace(/[_~]/u, m => dictHtml3[m]);
    const re = new RegExp(String.raw `\b(${config.protocol.split('|')
        .filter(p => p.endsWith(':'))
        .map(p => p.slice(0, -1))
        .join('|')}):`, 'giu');
    return output.replace(re, '$1&#58;');
}, pagenamee = (page) => encodeURI(page.replaceAll(' ', '_')), fullpagename = (title, config) => wfEscapeWikiText(title.prefix + title.main, config), fullpagenamee = (title, config) => wfEscapeWikiText(pagenamee(title.prefix + title.main), config), subpagename = (target, config) => {
 ...
 */
const expandMagicWord = (name, args, page = '', config = index_1.default.getConfig(), now = index_1.default.now, accum) => {
    const arg0 = args[0], target = args[0] ?? page;
    switch (name) {
        case 'currentyear':
        ...
        case 'localurle':
            return (0, string_1.escape)(localurl(config, args));
        case 'fullurl':
            return fullurl(config, args);
        case 'fullurle': {
            const url = fullurl(config, args);
            return url && (0, string_1.escape)(url);
        }
        case 'canonicalurl':
        ...
        case 'tag': {
            const tagName = strip(arg0).toLowerCase(), [, inner, ...attrs] = args, attributes = new Map();
        ...
        case 'titleparts': {
            ...
            const end = parts < 0 ? parts : offset + parts || undefined, bits = (title.prefix + title.main).split('/');
            if (bits.length > 25) {
        ...
        case 'replace': {
            const from = strip(args[1] ?? '') || ' ', to = strip(args[2] ?? ''), bits = strip(arg0).split(from), limit = Number(args[3]);
            return Number.isNaN(limit) || limit < 0 || limit >= bits.length - 1
                ? bits.join(to)
                : bits.slice(0, limit + 1).join(to) + from + bits.slice(limit + 1).join(from);
        }
        case 'explode': {
            const [, arg1 = '', arg2 = 0] = args, inPos = Number(arg2);
            if (arg2 === '' || Number.isNaN(inPos)) {
                return '';
            }
            const inDiv = strip(arg1) || ' ', bits = strip(arg0).split(inDiv), inLim = Number(args[3]);
        ...
        case 'ifexist': {
            ...
            const { valid, interwiki } = index_1.default.normalizeTitle((0, string_1.decodeHtml)(arg0), 0, false, config, { halfParsed: true, temporary: true, page: '' });
        ...
        case 'switch': {
            ...
            const v = (0, string_1.decodeHtml)(arg0);
            let defaultVal = '', j = 1, 
            /**
             * - `1` 表示默认值
             * - `2` 表示匹配值
            ...
            found = 0;
            for (; j < length; j++) {
                const arg = args[j], splitted = splitArg(arg);
                if (splitted) {
                    const [key, value] = splitted, known = isKnown(key);
...
constants_1.parsers['expandMagicWord'] = __filename;
