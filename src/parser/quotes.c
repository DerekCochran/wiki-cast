...
void parse_quotes(ThreadBuf *tb, const ParserConfig *cfg, Accum *accum, bool tidy)
{
    ...
    while (search_at <= tb->len) {
        ...
        if (parts_len >= parts_cap) { parts_cap *= 2; parts = realloc(parts, parts_cap * sizeof(Part)); assert(parts); }
    ...
    for (size_t k = 0; k < parts_len; k++) if (parts[k].is_quote) { any_quote = true; break; }
    if (!any_quote) {
    ...
    for (size_t i = 1; i + 0 < parts_len; i += 2) {
        ...
        char marker_buf[64]; size_t marker_len = 0;
    ...
    if (tidy && (bold_open || italic_open)) {
        ...
        char extra_buf[8]; size_t elen = 0;
        if (bold_open) { extra_buf[elen++] = '\''; extra_buf[elen++] = '\''; extra_buf[elen++] = '\''; }
        if (italic_open) { extra_buf[elen++] = '\''; extra_buf[elen++] = '\''; }
