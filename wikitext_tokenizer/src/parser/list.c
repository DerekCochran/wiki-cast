...
static char **split_on_semicolon_lookahead(const char *s, size_t len, size_t *out_count)
{
    ...
    for (size_t i = 0; i < len; i++) {
        if (s[i] == ';' && i != start) {
            ...
            p[plen] = '\0';
            if (count >= cap) { cap *= 2; parts = realloc(parts, cap * sizeof(char*)); assert(parts); }
            parts[count++] = p;
            start = i;
        }
    }
...
void parse_list(ThreadBuf *tb, const ParserConfig *cfg, Accum *accum)
{
    ...
    while (pos <= blen) {
        ...
        line[llen] = '\0';
        if (line_count >= line_cap) { line_cap *= 2; lines = realloc(lines, line_cap * sizeof(char*)); lines_len = realloc(lines_len, line_cap * sizeof(size_t)); assert(lines && lines_len); }
    ...
    for (size_t li = 0; li < line_count; li++) {
        ...
        if (rc <= 0) {
            ...
            if (lastPrefix) { free(lastPrefix); lastPrefix = strdup(""); }
            continue;
        }
        ...
        assert(out);
        size_t out_len = 0;
        if (comment_len) { memcpy(out + out_len, comment, comment_len); out_len += comment_len; }
        ...
        for (size_t i = 0; i < parts_count; i++) {
            ...
            if (out_len + mlen + 1 > out_cap) { out_cap = (out_len + mlen + 1) * 2; out = realloc(out, out_cap); assert(out); }
        ...
        if (out_len + rest_len + 1 > out_cap) { out_cap = out_len + rest_len + 1; out = realloc(out, out_cap); assert(out); }
        ...
        size_t search_at = 0;
        int lt = 0; int lc = 0; int lb = 0; int li_flag = 0;
        ...
        while (search_at <= out_len && dt > 0) {
            ...
            if (slen == 2 && out[mstart] == '}' && out[mstart+1] == '-') {
                lc--;
                if (!lc) { re_cur = re_full; md_cur = md_full; }
                search_at = mend;
                continue;
            }
            ...
            if ((unsigned char)out[mstart] == 0 && (unsigned char)out[mend-1] == 0x7F) {
                ...
                if (typech == 'x') {
                    ...
                    for (size_t ni = 0; ni < cfg->html[0].count; ni++) if (cfg->html[0].items[ni] && name && strcmp(cfg->html[0].items[ni], name) == 0) { is_normal = true; break; }
                    bool is_void = false;
                    for (size_t vi = 0; vi < cfg->html[2].count; vi++) if (cfg->html[2].items[vi] && name && strcmp(cfg->html[2].items[vi], name) == 0) { is_void = true; break; }
                    ...
                    if (bold) { if (!lb) lt++; else if (lt) lt--; lb = !lb; }
                    if (italic) { if (!li_flag) lt++; else if (lt) lt--; li_flag = !li_flag; }
                }
                search_at = mend;
                continue;
            }
            ...
            if (out[mstart] == ':') {
                ...
                if (lt == 0) {
                    ...
                    if (new_len + 1 > out_cap) { out = realloc(out, new_len + 1); assert(out); out_cap = new_len + 1; }
