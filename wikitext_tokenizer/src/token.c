...
Token *token_new(TokenType type, const char *type_name)
{
    ...
    if (!t->children) { free(t->type_name); free(t); return NULL; }
    return t;
}
    ...
    switch (t->type) {
        ...
        case TOKEN_REDIRECT_TARGET: {
            ...
            for (size_t i = 0; i < t->child_count; i++) {
                ...
                if (c->is_text) {
                    thread_buf_append(tb, c->text, c->text_len);
                } else if (t->type == TOKEN_FILE && c->token
                           && c->token->type == TOKEN_PLAIN
                           && c->token->type_name
        ...
        case TOKEN_PARAMETER: {
            ...
            if (t->child_count == 1) {
                ...
                if (only->is_text) thread_buf_append(tb, only->text, only->text_len);
                else token_to_string_rec(only->token, tb);
                return;
            }
            ...
            if (!key_empty) {
                if (k->is_text) thread_buf_append(tb, k->text, k->text_len);
                else token_to_string_rec(k->token, tb);
                thread_buf_append_char(tb, '=');
            }

            if (v->is_text) thread_buf_append(tb, v->text, v->text_len);
            else token_to_string_rec(v->token, tb);
            return;
        }
        ...
        case TOKEN_CONVERTER: {
            ...
            if (t->child_count > 0) {
                ...
                if (!fc->is_text && fc->token) {
                    ...
                    for (size_t i = 0; i < flags->child_count; i++) {
                        ...
                        if (f->is_text) thread_buf_append(tb, f->text, f->text_len);
                        else token_to_string_rec(f->token, tb);
                    }
                }
            }
            ...
            if (t->child_count > 1) {
                ...
                for (size_t i = 1; i < t->child_count; i++) {
                    ...
                    if (r->is_text) thread_buf_append(tb, r->text, r->text_len);
                    else token_to_string_rec(r->token, tb);
                }
            }
