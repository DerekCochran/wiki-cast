...
int main(void)
{
    ...
    size_t failed = run_parser_samples("comment_and_ext", samples, sizeof(samples) / sizeof(samples[0]) - 2, cfg, false, 0);

    /* last two samples must also survive all 10 stages */
    const char *full_stage_samples[] = {
        samples[sizeof(samples) / sizeof(samples[0]) - 2],
        samples[sizeof(samples) / sizeof(samples[0]) - 1],
    };
    failed += run_parser_samples("comment_and_ext_full", full_stage_samples, 2, cfg, false, 10);
